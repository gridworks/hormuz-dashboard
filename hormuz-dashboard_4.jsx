...
const Ticker = ({ value, unit, dec=1, color="#00c9a7", size="1.5rem" }) => {
...
const BrentSpark = ({ history, w=200, h=44 }) => {
  if (!history.length) return <div style={{ width:w, height:h, display:"flex", alignItems:"center", justifyContent:"center", color:"#556677", fontSize:"0.6rem" }}>Loading chart…</div>;
  ...
  return (
    <div style={{ background:"rgba(10,15,26,0.96)", border:`1px solid ${era.border}`, borderRadius:12, padding:"16px 20px 14px", marginBottom:16 }}>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12 }}>
        <div>
          <div style={{ color:"#556677", fontSize:"0.52rem", letterSpacing:3, fontFamily:"'Space Mono',monospace", textTransform:"uppercase" }}>SELECTED PERIOD</div>
          <div style={{ display:"flex", alignItems:"center", gap:7, marginTop:3, flexWrap:"wrap" }}>
            <span style={{ fontSize:"1rem", fontWeight:700, color:"#fff", fontFamily:"'Space Mono',monospace" }}>{sel.label}</span>
            <span style={{ padding:"2px 8px", borderRadius:20, fontSize:"0.58rem", fontFamily:"'Space Mono',monospace", background:era.bg, border:`1px solid ${era.border}`, color:era.color }}>{era.label}</span>
            <span style={{ padding:"2px 8px", borderRadius:20, fontSize:"0.58rem", fontFamily:"'Space Mono',monospace", background:SRC_COLOR[sel.src]==="EIA"?"rgba(0,201,167,0.1)":"rgba(240,165,0,0.1)", border:`1px solid ${SRC_COLOR[sel.src]}44`, color:SRC_COLOR[sel.src] }}>{SRC_LABEL[sel.src]}</span>
            {sel.id==="2026-03"&&<span style={{ padding:"2px 8px", borderRadius:20, fontSize:"0.58rem", fontFamily:"'Space Mono',monospace", background:"rgba(0,201,167,0.12)", border:"1px solid #00c9a7", color:"#00c9a7" }}>● TODAY</span>}
          </div>
        </div>
        <div style={{ display:"flex", gap:4, flexWrap:"wrap", justifyContent:"flex-end" }}>
          {[{l:"Jan '23",i:0},{l:"Oct '23",i:9},{l:"Jan '24",i:12},{l:"Jan '25",i:24},{l:"Today",i:months.length-1}].map(b=>(
            <button key={b.l} onClick={()=>onChange(b.i)} style={{ padding:"3px 9px", borderRadius:5, background:idx===b.i?"rgba(0,201,167,0.1)":"rgba(255,255,255,0.03)", border:`1px solid ${idx===b.i?era.color:"rgba(255,255,255,0.07)"}`, color:idx===b.i?era.color:"#6677aa", cursor:"pointer", fontSize:"0.6rem", fontFamily:"'Space Mono',monospace" }}>{b.l}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{ position:"relative", paddingBottom:20, marginBottom:4 }}>
        ...
        {yearPositions.map(({y,pct})=>(
          <div key={y} style={{ position:"absolute", bottom:0, left:`${pct}%`, transform:"translateX(-50%)", textAlign:"center" }}>
            ...
            <div style={{ fontSize:"0.55rem", color:"#556677", fontFamily:"'Space Mono',monospace" }}>{y}</div>
          </div>
        ))}
        <div style={{ position:"absolute", bottom:0, right:0, textAlign:"center" }}>
          ...
          <div style={{ fontSize:"0.52rem", color:"#00c9a7", fontFamily:"'Space Mono',monospace" }}>NOW</div>
        </div>
      ...
      {/* Event note */}
      {sel.note&&(
        <div style={{ padding:"6px 10px", background:era.bg, border:`1px solid ${era.border}`, borderRadius:5, fontSize:"0.68rem", color:era.color, fontFamily:"'Space Mono',monospace", marginBottom:10 }}>⚡ {sel.note}</div>
      )}
        ...
        {[
          ...
          return (
            <div key={i} style={{ background:"rgba(0,0,0,0.15)", borderRadius:7, padding:"8px 10px" }}>
              <div style={{ color:"#556677", fontSize:"0.5rem", letterSpacing:2, fontFamily:"'Space Mono',monospace", marginBottom:3 }}>{s.label}</div>
              ...
              <div style={{ marginTop:3, display:"flex", alignItems:"center", gap:7 }}>
                <span style={{ fontFamily:"'Space Mono',monospace", fontSize:"0.8rem", color:s.color }}>{months[idx][s.field].toFixed(s.dec)}<span style={{ fontSize:"0.6em", opacity:0.6, marginLeft:2 }}>{s.unit}</span></span>
                {idx>0&&<span style={{ fontSize:"0.56rem", color:parseFloat(delta)>=0?"#00c9a7":"#e84b3a" }}>{parseFloat(delta)>=0?"▲":"▼"}{Math.abs(delta)}% vs Jan'23</span>}
              </div>
            </div>
          );
        })}
      ...
      <div style={{ display:"flex", gap:14, marginTop:10, flexWrap:"wrap", alignItems:"center" }}>
        {Object.entries(ERA).map(([k,v])=>(
          <div key={k} style={{ display:"flex", alignItems:"center", gap:4, fontSize:"0.58rem", color:"#778899", fontFamily:"'Space Mono',monospace" }}>
            <div style={{ width:8, height:8, borderRadius:2, background:v.color, opacity:0.75 }}/>{v.label}
          </div>
        ))}
        <div style={{ marginLeft:"auto", display:"flex", gap:10 }}>
          <span style={{ fontSize:"0.55rem", color:"#00c9a7", fontFamily:"'Space Mono',monospace" }}>▓ EIA Verified</span>
          <span style={{ fontSize:"0.55rem", color:"#f0a500", fontFamily:"'Space Mono',monospace" }}>░ Modeled Est.</span>
        </div>
      </div>
    </div>
  );
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
        <div>
          <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:4 }}>
            ...
            <span style={{ color:"#00c9a7", fontFamily:"'Space Mono',monospace", fontSize:"0.55rem", letterSpacing:3, textTransform:"uppercase" }}>
              {brent.loading ? "FETCHING LIVE DATA…" : `LIVE · ${brent.source}`}
            </span>
          </div>
          <div style={{ fontSize:"0.62rem", color:"#556677", fontFamily:"'Space Mono',monospace", marginBottom:6 }}>BRENT CRUDE SPOT PRICE (USD/bbl)</div>
          {brent.loading
            ? <div style={{ color:"#778899", fontFamily:"'Space Mono',monospace", fontSize:"1.6rem" }}>—</div>
            : (
              <div style={{ display:"flex", alignItems:"baseline", gap:10 }}>
                <span style={{ color:"#f0a500", fontFamily:"'Space Mono',monospace", fontWeight:700, fontSize:"2rem" }}>${brent.price?.toFixed(2)}</span>
                {brent.change!=null&&(
                  <span style={{ fontSize:"0.85rem", color:up?"#00c9a7":"#e84b3a", fontFamily:"'Space Mono',monospace" }}>
          ...
          {brent.error&&<div style={{ fontSize:"0.6rem", color:"#f0a500", fontFamily:"'Space Mono',monospace", marginTop:4 }}>⚠ {brent.error}</div>}
          <div style={{ fontSize:"0.58rem", color:"#445566", fontFamily:"'Space Mono',monospace", marginTop:3 }}>
            {brent.ts ? `As of ${brent.ts}` : ""}
            {" · Updates every 5 min · Set VITE_EIA_API_KEY for official EIA data"}
          </div>
        </div>
        <div>
          <div style={{ fontSize:"0.52rem", color:"#556677", fontFamily:"'Space Mono',monospace", marginBottom:4 }}>52-WEEK BRENT (EIA weekly)</div>
          <BrentSpark history={history} w={200} h={44}/>
          {!history.length&&<div style={{ fontSize:"0.58rem", color:"#445566", fontFamily:"'Space Mono',monospace", marginTop:2 }}>Add VITE_EIA_API_KEY to .env for chart</div>}
        </div>
      </div>
    </div>
  );
...
const DataQualityBanner = ({ idx }) => {
  ...
  return (
    <div style={{ padding:"8px 14px", borderRadius:7, marginBottom:12, display:"flex", alignItems:"center", gap:10, background:bg, border:`1px solid ${color}44`, fontSize:"0.68rem", color:"#99aabb" }}>
      ...
      <div style={{ background:"linear-gradient(180deg,#0d1626,rgba(13,22,38,0.97))", borderBottom:"1px solid rgba(0,201,167,0.1)", padding:"14px 24px", display:"flex", justifyContent:"space-between", alignItems:"center", position:"relative", zIndex:10 }}>
        <div>
          <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:2 }}>
            ...
            <span style={{ color:"#00c9a7", fontFamily:"'Space Mono',monospace", fontSize:"0.55rem", letterSpacing:4, textTransform:"uppercase" }}>Maritime Intelligence · Mar 12, 2026</span>
          </div>
          <h1 style={{ margin:0, fontSize:"1.3rem", fontWeight:700, color:"#fff", letterSpacing:-0.4 }}>STRAIT OF HORMUZ — TRAFFIC INTELLIGENCE</h1>
          <div style={{ color:"#6677aa", fontSize:"0.65rem", marginTop:2, fontFamily:"'Space Mono',monospace" }}>
            EIA VERIFIED HISTORY · LIVE BRENT · AIS + LLOYD'S BASELINES · JAN 2023 → TODAY
          </div>
        </div>
        <div style={{ textAlign:"right" }}>
          <div style={{ padding:"5px 12px", background:era.bg, border:`1px solid ${era.border}`, borderRadius:20, fontSize:"0.65rem", fontFamily:"'Space Mono',monospace", color:era.color, marginBottom:4 }}>
            {sel.label} · {era.label}
          </div>
          <div style={{ fontSize:"0.58rem", color:SRC_COLOR[sel.src], fontFamily:"'Space Mono',monospace" }}>{SRC_LABEL[sel.src]}</div>
        </div>
      </div>
          ...
          return (
            <div key={i} style={{ padding:"12px 16px", borderRight:i<4?"1px solid rgba(0,201,167,0.05)":"none", background:"rgba(10,15,26,0.85)" }}>
              <div style={{ display:"flex", alignItems:"center", gap:5, marginBottom:3 }}>
                <div style={{ color:"#556677", fontSize:"0.5rem", letterSpacing:3, textTransform:"uppercase", fontFamily:"'Space Mono',monospace" }}>{k.label}</div>
              ...
              {d!=null&&idx>0&&<div style={{ fontSize:"0.55rem", color:parseFloat(d)>=0?"#00c9a7":"#e84b3a", marginTop:2, fontFamily:"'Space Mono',monospace" }}>{parseFloat(d)>=0?"▲":"▼"}{Math.abs(d)}%</div>}
              {k.live&&brent.change!=null&&<div style={{ fontSize:"0.55rem", color:brent.change>=0?"#00c9a7":"#e84b3a", marginTop:2, fontFamily:"'Space Mono',monospace" }}>{brent.change>=0?"▲":"▼"}{Math.abs(brent.change).toFixed(2)} today</div>}
            </div>
          );
        })}
      </div>
      ...
      <div style={{ padding:"18px 24px", position:"relative", zIndex:10 }}>
                ...
                {[
                  ...
                  <div key={i}>
                    <div style={{ display:"flex", alignItems:"center", gap:4, marginBottom:2 }}>
                      <div style={{ color:"#556677", fontSize:"0.52rem", letterSpacing:2, fontFamily:"'Space Mono',monospace", textTransform:"uppercase" }}>{s.l}</div>
                      <div style={{ fontSize:"0.45rem", color:SRC_COLOR[s.src]||"#778899", fontFamily:"'Space Mono',monospace", opacity:0.8 }}>[{s.src}]</div>
                    </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
              <div style={{ background:"rgba(13,22,38,0.8)", border:"1px solid rgba(255,255,255,0.04)", borderRadius:11, padding:"16px 18px" }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:13 }}>
                  <div style={{ color:"#556677", fontSize:"0.52rem", letterSpacing:3, fontFamily:"'Space Mono',monospace" }}>CRUDE EXPORT SHARE BY PRODUCER</div>
                  <span style={{ fontSize:"0.5rem", color:"#00c9a7", fontFamily:"'Space Mono',monospace" }}>✓ EIA 2024</span>
                </div>
                {PRODUCERS.map((p,i)=>(
              ...
              <div style={{ background:"rgba(13,22,38,0.8)", border:"1px solid rgba(255,255,255,0.04)", borderRadius:11, padding:"16px 18px" }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:13 }}>
                  <div style={{ color:"#556677", fontSize:"0.52rem", letterSpacing:3, fontFamily:"'Space Mono',monospace" }}>CARGO DESTINATION BREAKDOWN</div>
                  <span style={{ fontSize:"0.5rem", color:"#00c9a7", fontFamily:"'Space Mono',monospace" }}>✓ EIA 2024 / IEA</span>
                </div>
          ...
          <div>
            <div style={{ display:"grid", gridTemplateColumns:"3fr 2fr", gap:14, marginBottom:14 }}>
              <div style={{ background:"rgba(13,22,38,0.8)", border:"1px solid rgba(255,255,255,0.04)", borderRadius:11, padding:"16px 18px" }}>
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:13 }}>
                  <div style={{ color:"#556677", fontSize:"0.52rem", letterSpacing:3, fontFamily:"'Space Mono',monospace" }}>VESSEL TYPE COMPOSITION (SCALED TO PERIOD)</div>
                  <span style={{ fontSize:"0.5rem", color:"#f0a500", fontFamily:"'Space Mono',monospace" }}>~ Lloyd's/AIS baseline</span>
                </div>
                {VESSEL_TYPES.map((v,i)=>(
                  <div key={i} style={{ display:"grid", gridTemplateColumns:"22px 180px 48px 70px 1fr 48px", alignItems:"center", gap:8, padding:"7px 0", borderBottom:"1px solid rgba(255,255,255,0.03)" }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize:"0.58rem", color:"#556677", marginTop:1 }}>{v.sizeM}m · {(v.avgDwt/1000).toFixed(0)}k DWT</div>
                    </div>
                    <span style={{ fontFamily:"'Space Mono',monospace", fontSize:"0.68rem", color:v.color, textAlign:"right" }}>{Math.round(v.count*ratio)}</span>
                    <span style={{ fontSize:"0.58rem", color:"#778899", textAlign:"center" }}>{v.direction}</span>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                <div style={{ background:"rgba(13,22,38,0.8)", border:"1px solid rgba(255,255,255,0.04)", borderRadius:11, padding:"16px 18px", flex:1 }}>
                  <div style={{ color:"#556677", fontSize:"0.52rem", letterSpacing:3, fontFamily:"'Space Mono',monospace", marginBottom:12 }}>TANKER SIZE CLASSES</div>
                  {[
                    ...
                    <div key={i} style={{ display:"flex", justifyContent:"space-between", padding:"5px 0", borderBottom:"1px solid rgba(255,255,255,0.03)", fontSize:"0.68rem" }}>
                      ...
                      <span style={{ color:"#556677", fontSize:"0.58rem" }}>{s.note}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div style={{ background:"rgba(13,22,38,0.8)", border:"1px solid rgba(255,255,255,0.04)", borderRadius:11, padding:"16px 18px" }}>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:12 }}>
                <div style={{ color:"#556677", fontSize:"0.52rem", letterSpacing:3, fontFamily:"'Space Mono',monospace" }}>FLAG STATE REGISTRY (AIS / LLOYD'S 2023–2024 BASELINE)</div>
                <span style={{ fontSize:"0.5rem", color:"#f0a500", fontFamily:"'Space Mono',monospace" }}>~ WorldwideAIS / Lloyd's List Intelligence</span>
              </div>
              <div style={{ display:"grid", gridTemplateColumns:"repeat(6,1fr)", gap:7 }}>
                {FLAGS.map((f,i)=>(
                  <div key={i} onMouseEnter={()=>setHFlag(i)} onMouseLeave={()=>setHFlag(null)}
                    ...
                    <div style={{ fontSize:"0.55rem", color:"#556677", marginTop:2 }}>{f.pct}% · {f.type}</div>
              ...
              {CARGO.map((c,i)=>(
                <div key={i} style={{ background:"rgba(13,22,38,0.8)", border:`1px solid ${c.color}18`, borderLeft:`3px solid ${c.color}`, borderRadius:9, padding:"13px 15px", display:"grid", gridTemplateColumns:"32px 1fr auto", gap:11, alignItems:"center" }}>
                  ...
                  <div style={{ textAlign:"right" }}>
                    ...
                    <div style={{ fontSize:"0.55rem", color:"#556677", marginTop:1 }}>annual est.</div>
                    {ratio<0.97&&<div style={{ fontSize:"0.55rem", color:"#e84b3a", marginTop:1 }}>↓{((1-ratio)*100).toFixed(0)}%</div>}
                  </div>
                </div>
              ))}
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:11 }}>
              {[
                ...
                <div key={i} style={{ background:"rgba(13,22,38,0.8)", border:"1px solid rgba(255,255,255,0.04)", borderRadius:9, padding:"14px 18px", textAlign:"center" }}>
                  <div style={{ color:"#556677", fontSize:"0.52rem", letterSpacing:3, fontFamily:"'Space Mono',monospace", marginBottom:7 }}>{s.l}</div>
        ...
        {tab==="origins & destinations"&&(
          <div>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
              <div style={{ background:"rgba(13,22,38,0.8)", border:"1px solid rgba(255,255,255,0.04)", borderRadius:11, padding:"16px 18px" }}>
                <div style={{ color:"#556677", fontSize:"0.52rem", letterSpacing:3, fontFamily:"'Space Mono',monospace", marginBottom:13 }}>MAJOR LOADING TERMINALS</div>
                {[
                  ...
                  <div key={i} style={{ padding:"7px 0", borderBottom:"1px solid rgba(255,255,255,0.03)" }}>
                    <div style={{ display:"flex", justifyContent:"space-between", marginBottom:2 }}>
                      ...
                      <div style={{ display:"flex", gap:6, alignItems:"center" }}>
                        ...
                        <span style={{ fontSize:"0.48rem", color:SRC_COLOR[p.src]||"#778899", fontFamily:"'Space Mono',monospace" }}>[{p.src}]</span>
                      </div>
                    </div>
              ...
              <div style={{ background:"rgba(13,22,38,0.8)", border:"1px solid rgba(255,255,255,0.04)", borderRadius:11, padding:"16px 18px" }}>
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:13 }}>
                  <div style={{ color:"#556677", fontSize:"0.52rem", letterSpacing:3, fontFamily:"'Space Mono',monospace" }}>DESTINATION FLOWS (PERIOD-ADJUSTED)</div>
                  <span style={{ fontSize:"0.48rem", color:"#00c9a7", fontFamily:"'Space Mono',monospace" }}>✓ EIA 2024</span>
                </div>
                {[
                  ...
                  <div key={i} style={{ padding:"8px 0", borderBottom:"1px solid rgba(255,255,255,0.03)" }}>
                    <div style={{ display:"flex", justifyContent:"space-between", marginBottom:3 }}>
                      ...
                      <span style={{ fontSize:"0.58rem", color:d.dep==="CRITICAL"?"#e84b3a":d.dep==="HIGH"?"#f0a500":"#778899", fontFamily:"'Space Mono',monospace" }}>{d.dep}</span>
                    </div>
                    <div style={{ display:"flex", gap:14, fontSize:"0.63rem", color:"#778899" }}>
                      ...
                      <span style={{ marginLeft:"auto", fontSize:"0.48rem", color:SRC_COLOR[d.src]||"#778899", fontFamily:"'Space Mono',monospace" }}>[{d.src}]</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div style={{ marginTop:12, background:"rgba(0,201,167,0.02)", border:"1px solid rgba(0,201,167,0.07)", borderRadius:9, padding:"13px 16px" }}>
              <div style={{ color:"#00c9a7", fontSize:"0.55rem", letterSpacing:3, fontFamily:"'Space Mono',monospace", marginBottom:7 }}>📡 DATA SOURCES, ACCURACY & LIVE WIRING</div>
